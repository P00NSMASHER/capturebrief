...
def apply_history_receipt(case: dict[str, Any], receipt: dict[str, Any]) -> dict[str, Any]:
    """Apply Data Services history evidence without treating it as currentness authority."""
    ...
    if str(receipt.get("automation_mode") or "") != "APPROVED_EXTRACT":
        raise EvidenceApplyError("history receipt is not marked APPROVED_EXTRACT")
    ...
    if receipt.get("history_set_sha256") != history_set_digest(actions):
        raise EvidenceApplyError("history receipt action-set digest is invalid")
    if not valid_sha256(receipt.get("evidence_payload_sha256")):
        raise EvidenceApplyError("history receipt evidence payload digest is invalid")
    if not parse_dt(receipt.get("observed_at")):
        raise EvidenceApplyError("history receipt observation time is invalid")
    ...
    # A changed action set invalidates an old current-action receipt. Preserve it as provenance
    # rather than silently keeping stale authority active.
    active_current = []
...
def apply_current_api_evidence(
    ...
    if verdict != "CURRENT_VERIFIED" or verified_action != action_id:
        ...
        raise EvidenceApplyError(
            f"current receipt does not validate against the case history: {codes}"
        )
    ...
    *,
    ...
    existing = next(
        (
            ...
            if str(a.get("artifact_id") or "") == aid
        ),
        None,
    )
    artifact = {
